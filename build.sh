...
BuildWithOptions()
{
	...
	cp --remove-destination $LibName $SdlDir/lib/Linux/$TARGET_ARCH/$SdlLibName
	popd
}
...
BuildWithOptions libSDL2-2.0d.so $BUILD_DIR-Shared-Debug ../$SDL_DIR libSDL2.so -DSDL_SHARED=ON -DCMAKE_BUILD_TYPE=Debug -DVIDEO_VULKAN=OFF -DVIDEO_MIR=OFF -DVIDEO_KMSDRM=OFF -DCMAKE_C_FLAGS=-gdwarf-4
BuildWithOptions libSDL2-2.0.so $BUILD_DIR-Shared-Release ../$SDL_DIR libSDL2.so -DSDL_SHARED=ON -DCMAKE_BUILD_TYPE=Release -DVIDEO_VULKAN=OFF -DVIDEO_MIR=OFF -DVIDEO_KMSDRM=OFF -DCMAKE_C_FLAGS=-gdwarf-4
