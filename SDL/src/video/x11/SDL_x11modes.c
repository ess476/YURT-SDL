...
X11_InitModes_XRandR(_THIS)
{
    ...
    int screen, i, n;
    int start = 0;
    const char* sdl_display_start = SDL_GetHint("SDL_DISPLAY_START");

    if (sdl_display_start)
    {
        start = atoi(sdl_display_start);
    }

    fprintf(stderr, "%s: Display: %s\n", __FUNCTION__, X11_XDisplayString(dpy));

    for (looking_for_primary = 1; looking_for_primary >= 0; looking_for_primary--) {
        for (screen = start; screen < screencount; screen++) {
